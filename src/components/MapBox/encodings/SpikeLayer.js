...
void main() {
  ...
  if (a_pos.z == 0.0) {
    p.y += a_colorAndHeight.a * u_yPixelToClip * p.w * u_factor;
  } else {
    p.x += a_pos.z * u_size * p.w * u_factor;
  }
  p.x += u_jitter.x * p.w;
  p.y += u_jitter.y * p.w;
  gl_Position = p;
...
export class SpikeLayer {
  ...
  onAdd(map, gl) {
    ...
    this._uFactor = gl.getUniformLocation(this._program, 'u_factor');
  ...
  render(gl, matrix) {
    ...
    gl.uniform1f(this._uFactor, this.factor);
