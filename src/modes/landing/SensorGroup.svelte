<script>
  ...
  $: sensors = group.sensors.filter((d) => d.highlight && d.highlight.includes('landing')).sort((a, b) => {
     if (a.isCasesOrDeath) {
...
<li>
  ...
  {#each sensors as sensor, i}
    {i === sensors.length -1 ? ' and' : (i > 0 ? ',' : '')} <a href="?mode={modeByID.indicator.id}&sensor={sensor.key}" on:click|preventDefault={() => showDetails(sensor)}
      >{sensor.name}</a
    >
  {/each}
</li>
